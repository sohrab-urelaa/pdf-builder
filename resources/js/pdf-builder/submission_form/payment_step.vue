<template>
    ...
    <div v-if="field.description" dir="auto" class="mb-4 px-1">
    ...
    <div>
        <input
            ...
            class="hidden"
        />
        <div v-if="modelValue && !sessionId" class="text-2xl mb-2">
            {{ t("already_paid") }}
        </div>
        <div v-else>
            <button v-if="sessionId" disabled class="base-button w-full">
                <IconLoader width="22" class="animate-spin" />
            ...
            >
                <IconInnerShadowTop
                    ...
                    class="animate-spin"
                />
                <IconBrandStripe v-else width="22" />
                <span>
                    {{ t("pay_with_strip") }}
                </span>
            </button>
...
export default {
    ...
    methods: {
        async submit() {
            if (this.sessionId) {
                return fetch(
                    this.baseUrl + "/api/stripe_payments/" + this.sessionId,
                    {
                        method: "PUT",
                        body: JSON.stringify({
