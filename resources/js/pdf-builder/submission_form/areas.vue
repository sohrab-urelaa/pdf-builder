<template>
    <template v-for="step in steps" :key="step[0].uuid">
        <template v-for="(field, fieldIndex) in step" :key="field.uuid">
            <template v-for="(area, areaIndex) in field.areas" :key="areaIndex">
                <Teleport
...
export default {
    ...
    methods: {
        findPageElementForArea(area) {
            ...
            ).getElementById(`page-${area.attachment_uuid}-${area.page}`);
            return element;
        },
        ...
        scrollIntoArea(area) {
            ...
            if (areaRef) {
                ...
                if (container.style.overflow === "hidden") {
                    ...
                    const targetRect =
                        areaRef.$refs.scrollToElem.getBoundingClientRect();
                    const root =
                        this.$root.$el?.parentNode?.classList?.contains("ds")
                            ? this.$root.$el
                            : document.body;
