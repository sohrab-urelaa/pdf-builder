<template>
    <div
        ...
        :class="{
            ...
            'cursor-default': !submittable,
            'border border-red-100 bg-red-100 cursor-pointer': submittable,
            'border border-red-100': !isActive && submittable,
            'bg-opacity-80': !isActive && !isValueSet && submittable,
            'field-area-active outline-red-500 outline-dashed outline-2 z-10':
                isActive && submittable,
            'bg-opacity-40': (isActive || isValueSet) && submittable,
        }"
    >
        <div
            v-if="
        ...
        >
            ...
            <template v-else>
                ...
                <template
                    v-else-if="!field.required && field.type !== 'checkbox'"
                >
                    (optional)
                </template>
            </template>
        ...
        >
            <div class="flex-grow flex overflow-hidden" style="min-height: 50%">
                <img class="object-contain mx-auto" :src="signature.url" />
            </div>
            <div
                v-if="withSignatureId"
                class="w-full mt-1 text-[1vw] lg:text-[0.55rem] lg:leading-[0.65rem]"
            >
                <div class="truncate uppercase">ID: {{ signature.uuid }}</div>
                <div>
                    {{ t("reason") }}: {{ t("digitally_signed_by") }}
                    {{ submitter.name }}
        ...
        <div
            v-else-if="
                (field.type === 'file' || field.type === 'payment') &&
                attachments.length
            "
        ...
        >
            <input
                ...
                :class="{
                    'base-radio':
                        !modelValue || modelValue !== optionValue(option),
                    '!w-auto !h-full': area.w > area.h,
                    '!w-full !h-auto': area.w <= area.h,
                }"
        ...
        >
            <input
                ...
                :checked="
                    !!modelValue && modelValue.includes(optionValue(option))
                "
        ...
        attachments() {
            if (this.field.type === "file") {
                return (this.modelValue || []).map(
                    (uuid) => this.attachmentsIndex[uuid]
                );
            ...
            if (
                ...
                this.textOverflowChars =
                    ...
                    this.$refs.textContainer.clientHeight
                        ? `${
                              this.modelValue ||
                              (this.withFieldPlaceholder ? this.field.name : "")
                          }`.length
                        : 0;
            }
        });
