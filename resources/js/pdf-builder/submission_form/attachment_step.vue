<template>
    <div>
        <div v-if="value.length">
            <div v-for="(val, index) in value" :key="index" class="flex mb-2">
                <input
                    :value="val"
                    type="hidden"
                ...
                <a
                    ...
                    target="_blank"
                >
                    <IconPaperclip :width="16" class="flex-none" :heigh="16" />
                    <span>
                        {{ attachmentsIndex[val].filename }}
                    </span>
                </a>
                <button @click.prevent="removeAttachment(val)">
                    <IconTrashX :width="18" :heigh="19" />
                </button>
            </div>
        </div>
        <template v-else>
        ...
        <FileDropzone
            :message="`${t('upload')} ${field.name || t('files')}${
...
<script>
import FileDropzone from "./dropzone.vue";
