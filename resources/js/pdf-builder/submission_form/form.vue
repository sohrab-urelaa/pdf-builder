<template>
    <!-- <Builder :withFieldsList="false" :template="template" /> -->
    <FormViewer :template="template">
        ...
        <button
            ...
            @click.prevent="
                [(isFormVisible = true), scrollIntoField(currentField)]
            "
        >
            <template
                v-if="['initials', 'signature'].includes(currentField?.type)"
            >
        ...
        >
            ...
            <div :class="{ 'md:px-4': isBreakpointMd }">
                <form
                    v-if="!isCompleted"
                    ref="form"
                    :action="submitPath"
                ...
                >
                    ...
                    <div class="md:mt-4">
                        <div
                            v-if="['cells', 'text'].includes(currentField.type)"
                        >
                            <TextStep
                        ...
                        <div v-else-if="currentField.type === 'select'">
                            <label
                                v-if="
                                    showFieldNames &&
                                    (currentField.name || currentField.title)
                                "
                            ...
                            >
                                <MarkdownContent
                                    :string="currentField.description"
                                />
                            </div>
                        ...
                        <MultiSelectStep
                            ...
                            :show-field-names="showFieldNames"
                            :is-last-step="
                                stepFields.length === currentStep + 1
                            "
                            :field="currentField"
                        />
                        ...
                        <SignatureStep
                            ...
                            :field="currentField"
                            :previous-value="
                                previousSignatureValueFor(currentField) ||
                        ...
                        <PhoneStep
                            ...
                            :show-field-names="showFieldNames"
                            :verified-value="
                                phoneVerifiedValues[currentField.uuid]
                            "
                    ...
                    <div
                        v-if="
                            ...
                            submittedValues[currentField.uuid]
                        "
                        :class="
                ...
                <FormCompleted
                    ...
                    :template="template"
                />
                <div v-if="stepFields.length < 80" class="flex justify-center">
                    <div class="flex items-center mt-4 mb-1 flex-wrap">
                        <a
                            ...
                            :class="{
                                'bg-base-300': index === currentStep,
                                'bg-base-content':
                                    (index < currentStep &&
                                        stepFields[index].every(
                                            (f) =>
                                                !f.required ||
...
<script>
import FieldAreas from "./areas.vue";
import FormulaFieldAreas from "./formula_areas.vue";
import ImageStep from "./image_step.vue";
import SignatureStep from "./signature_step.vue";
import InitialsStep from "./initials_step.vue";
import AttachmentStep from "./attachment_step.vue";
import MultiSelectStep from "./multi_select_step.vue";
import PhoneStep from "./phone_step.vue";
import PaymentStep from "./payment_step.vue";
import TextStep from "./text_step.vue";
import NumberStep from "./number_step.vue";
import DateStep from "./date_step.vue";
import MarkdownContent from "./markdown_content.vue";
import FormCompleted from "./completed.vue";
...
export default {
    ...
    computed: {
        ...
        previousInitialsValue() {
            if (this.reuseSignature !== false) {
                const initialsField = [...this.fields]
                    .reverse()
                    .find(
                        (field) =>
                            field.type === "initials" &&
                            !!this.values[field.uuid]
                    );
        ...
        isAnonymousChecboxes() {
            return (
                this.currentField.type === "checkbox" &&
        ...
        isButtonDisabled() {
            if (this.recalculateButtonDisabledKey) {
                return (
                    ...
                    (this.currentField.required &&
                        this.currentField.type === "signature" &&
                        !this.values[this.currentField.uuid]?.length &&
                        this.$refs.currentStep &&
                        !this.$refs.currentStep.isSignatureStarted) ||
                    (this.currentField.required &&
                        this.currentField.type === "initials" &&
                        !this.values[this.currentField.uuid]?.length &&
                        this.$refs.currentStep &&
            ...
            const steps = this.fields
                ...
                .reduce((acc, f) => {
                    ...
                    if (this.checkFieldConditions(f)) {
                        if (
                            f.type === "checkbox" &&
                            Array.isArray(prevStep) &&
                            prevStep[0].type === "checkbox" &&
                            !f.description
                        ) {
                            prevStep.push(f);
                        } else {
    ...
    async mounted() {
        ...
        if (this.queryParams.get("field_uuid")) {
            const stepIndex = this.stepFields.findIndex((fields) => {
                return fields.some(
                    (f) => f.uuid === this.queryParams.get("field_uuid")
                );
            });

            this.currentStep = Math.max(stepIndex, 0);
        } else if (this.goToLast) {
            const requiredEmptyStepIndex = this.stepFields.indexOf(
                this.stepFields.find((fields) =>
                    fields.some(
                        (f) => f.required && !this.submittedValues[f.uuid]
                    )
                )
            );
            const lastFilledStepIndex =
                this.stepFields.indexOf(
                    [...this.stepFields]
                        .reverse()
                        .find((fields) =>
                            fields.some((f) => !!this.submittedValues[f.uuid])
                        )
                ) + 1;
        ...
        if (/iPhone|iPad|iPod/i.test(navigator.userAgent)) {
            this.$nextTick(() => {
                ...
                scrollbox.parentNode.classList.add(
                    "h-screen",
                    "overflow-y-auto"
                );
    ...
    methods: {
        signatureUploaded(cb) {
            socket.on("signed_data", (result) => {
                const data = result?.data;

                if (data?.url || data?.base64) {
                    console.log("Data", data);
                    console.log("Values", this.values[data.fielduuid]);
                    console.log(
                        "Values",
                        this.attachments.find(
                            (item) => item.uuid === data.fielduuid
                        )
                    );
                    if (data?.uuid) {
                        this.attachments.push(data);
                        this.values[data.fielduuid] = data.uuid;
                    }
                }
            });
        },
        t(key) {
            return (
                this.i18n[key] ||
                i18n[this.language?.toLowerCase()]?.[key] ||
                i18n[this.browserLanguage]?.[key] ||
                i18n.en[key] ||
                key
            );
        },
            ...
            if (field.conditions?.length) {
                return field.conditions.reduce((acc, c) => {
                    if (["empty", "unchecked"].includes(c.action)) {
                        ...
                        ["not_equal", "does_not_contain"].includes(c.action)
                    ) {
                        const field = this.fieldsUuidIndex[c.field_uuid];
                        const option = field.options.find(
                            (o) => o.uuid === c.value
        ...
        previousSignatureValueFor(field) {
            if (this.reuseSignature !== false) {
                const signatureField = [...this.fields]
                    .reverse()
                    .find(
                        (f) =>
                            f.type === "signature" &&
                            field.preferences?.format ===
                                f.preferences?.format &&
                            !!this.values[f.uuid]
                    );
            ...
            if (this.dryRun) {
                ...
                currentFieldUuids.forEach((fieldUuid) => {
                    ...
                    if (currentFieldType === "phone") {
                        this.phoneVerifiedValues[fieldUuid] =
        ...
        async submitStep() {
            ...
            const stepPromise = [
                ...
                "initials",
                "payment",
            ].includes(this.currentField.type)
                ? this.$refs.currentStep.submit
                : () => Promise.resolve({});

            stepPromise()
                .then(async () => {
                    const emptyRequiredField = this.stepFields.find(
                        (fields, index) => {
                            ...
                            return fields.some((f) => {
                                return (
                                    f.required &&
                                    (f.type === "phone" || !this.allowToSkip) &&
                                    isEmpty(this.submittedValues[f.uuid])
                                );
                            });
                        }
                    );
                    ...
                    const isLastStep =
                        this.currentStep === this.stepFields.length - 1;
                    ...
                    const nextStep =
                        (isLastStep && emptyRequiredField) ||
                        this.stepFields[this.currentStep + 1];
