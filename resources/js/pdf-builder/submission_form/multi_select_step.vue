<template>
    ...
    <div class="flex w-full max-h-44 overflow-y-auto">
        <input
            ...
            class="hidden"
        />
        <div v-if="!showOptions" class="text-xl px-1">
            <span @click="scrollIntoField(field)">
                {{ t("complete_hightlighted_checkboxes_and_click") }}
        ...
        <div class="space-y-3.5 mx-auto" :class="{ hidden: !showOptions }">
            <div v-for="(option, index) in field.options" :key="option.uuid">
                <label
                    :for="option.uuid"
    ...
    methods: {
        ...
        onChange() {
            this.$emit(
                ...
                this.inputRefs
                    .filter((e) => e.checked)
                    .map((e, index) => this.optionValue(e, index))
            );
        },
    },
};
