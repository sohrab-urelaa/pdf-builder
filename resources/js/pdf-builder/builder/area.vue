<template>
    ...
    >
        <div
            ...
            :class="
                field.type === 'heading' ? '' : borderColors[submitterIndex]
            "
        />
        <div
        ...
        >
            <FieldSubmitter
                ...
                :editable="
                    editable &&
                    (!defaultField || defaultField.role !== submitter?.name)
                "
            ...
            <span
                ...
                ref="name"
                :contenteditable="
                    editable && !defaultField && field.type !== 'heading'
                "
                dir="auto"
            ...
            <div
                v-if="
                    isSettingsFocus ||
                    (isValueInput && field.type !== 'heading') ||
        ...
        <div
            ...
            dir="auto"
            :class="[
                isValueInput ? 'bg-opacity-50' : 'bg-opacity-80',
                field.type === 'heading'
                    ? 'bg-gray-50'
                    : bgColors[submitterIndex],
                isDefaultValuePresent ||
                isValueInput ||
                (withFieldPlaceholder && field.areas)
        ...
        >
            <span
                ...
                :class="{
                    'w-full': ['cells', 'checkbox'].includes(field.type),
                    'h-full': !isValueInput,
                }"
            >
                <div
                    ...
                    "
                    :class="{
                        'w-full h-full': ['cells', 'checkbox'].includes(
                            field.type
                        ),
                        'text-[1.6vw] lg:text-base': !textOverflowChars,
                ...
                >
                    <div
                        ...
                        :style="computedFieldPreferencesStyle"
                    >
                        <IconCheck
                        ...
                        >
                        <span v-else-if="field.default_value === '{{date}}'">
                            {{ t("signing_date") }}
                        </span>
                        <div
                            v-else-if="
                                field.type === 'cells' && field.default_value
                            "
                        ...
                        <span
                            ...
                            :placeholder="
                                withFieldPlaceholder && !isValueInput
...
<script>
import FieldSubmitter from "../builder/field_submitter.vue";
import FieldType from "../builder/field_type.vue";
import Field from "../builder/field.vue";
import FieldSettings from "../builder/field_settings.vue";
import FormulaModal from "../builder/formula_modal.vue";
import ConditionsModal from "../builder/conditions_modal.vue";
import DescriptionModal from "../builder/description_modal.vue";
...
export default {
    ...
    computed: {
        computedFieldPreferencesStyle() {
            const style = {};
            if (this.field.preferences?.font_size) {
                style.fontSize = this.field.preferences.font_size + "pt";
            }
            if (this.field.preferences?.font_color) {
                style.color = this.field.preferences.font_color;
            }
            return style;
        },
        ...
        isDefaultValuePresent() {
            if (this.field?.type === "radio" && this.field?.areas?.length > 1) {
                ...
                return (
                    this.field?.default_value || this.field?.default_value === 0
                );
            }
        },
        isValueInput() {
            return (
                (this.field.type === "heading" && this.isSelected) ||
                this.isContenteditable ||
                (this.inputMode &&
                    ["text", "number", "date"].includes(this.field.type))
            );
        },
        modalContainerEl() {
            return this.$el
                .getRootNode()
                .querySelector("#docuseal_modal_container");
        },
        defaultName() {
    ...
    methods: {
        ...
        focusValueInput(e) {
            if (this.$refs.defaultValue !== document.activeElement) {
                ...
                if (
                    ...
                    this.$refs.defaultValue !== e?.target
                ) {
                    window
                        .getSelection()
                        .collapse(
        ...
        startTouchDrag(e) {
            if (
                e.target !== this.$refs.touchTarget &&
                e.target !== this.$refs.touchValueTarget
            ) {
                return;
            }
        ...
        touchDrag(e) {
            ...
            this.area.x = Math.min(
                Math.max(
                    (this.dragFrom.x + e.touches[0].clientX - rect.left) /
                        rect.width,
                    0
                ),
                1 - this.area.w
            );
            this.area.y = Math.min(
                Math.max(
                    (this.dragFrom.y + e.touches[0].clientY - rect.top) /
                        rect.height,
                    0
                ),
                1 - this.area.h
        ...
        startMouseMove(e) {
            ...
            this.$el
                .getRootNode()
                .addEventListener("mouseup", this.stopMouseMove);
        ...
        mouseMove(e) {
            ...
            this.area.x = Math.min(
                Math.max(
                    (this.dragFrom.x + e.clientX - rect.left) / rect.width,
                    0
                ),
                1 - this.area.w
            );
            this.area.y = Math.min(
                Math.max(
                    (this.dragFrom.y + e.clientY - rect.top) / rect.height,
                    0
                ),
                1 - this.area.h
            );
        ...
        stopMouseMove(e) {
            ...
            this.$el
                .getRootNode()
                .removeEventListener("mouseup", this.stopMouseMove);
