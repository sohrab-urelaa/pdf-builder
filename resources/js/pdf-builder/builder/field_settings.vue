<template>
    ...
    >
        <select
            ...
            @change="
                [
                    (field.preferences ||= {}),
                    (field.preferences.format = $event.target.value),
                    save(),
                ]
            "
        >
    ...
    >
        <select
            ...
            @change="
                [
                    (field.preferences ||= {}),
                    (field.preferences.align = $event.target.value),
                    save(),
                ]
            "
        >
            <option
                ...
                :key="value"
                :selected="
                    field.preferences?.align
                        ? value === field.preferences.align
                        : value === 'left'
                "
                :value="value"
            >
    ...
    >
        <select
            ...
            @change="
                [
                    (field.default_value = $event.target.value),
                    !field.default_value && delete field.default_value,
                    save(),
                ]
            "
        >
            <option value="" :selected="!field.default_value">
                {{ t("none") }}
            </option>
            <option
    ...
    >
        ...
        >
            <option :selected="!field.validation" value="">
                {{ t("none") }}
            </option>
            <option
            ...
            <option
                :selected="
                    field.validation && !validations[field.validation.pattern]
                "
                :value="
    ...
    <div
        v-if="
            ['text', 'cells'].includes(field.type) &&
            field.validation &&
            !validations[field.validation.pattern]
        "
        class="py-1.5 px-1 relative"
    ...
    <li v-if="field.type == 'stamp'" @click.stop>
        <label class="cursor-pointer py-1.5">
            <input
                ...
                @change="
                    [
                        (field.preferences ||= {}),
                        (field.preferences.with_logo =
                            field.preferences.with_logo == false),
                        save(),
                    ]
                "
            />
    ...
    <li v-if="field.type == 'checkbox'" @click.stop>
        <label class="cursor-pointer py-1.5">
            <input
                v-model="field.default_value"
                type="checkbox"
    ...
    <li v-if="field.type == 'date'" @click.stop>
        <label class="cursor-pointer py-1.5">
            <input
                ...
                @update:model-value="
                    [
                        (field.default_value = $event ? '{{date}}' : null),
                        (field.readonly = $event),
                        save(),
                    ]
                "
    ...
    <hr v-if="field.type != 'stamp'" class="pb-0.5 mt-0.5" />
    ...
    <!-- <li v-if="field.type != 'stamp'">
        ...
        >
            ...
            <span class="text-sm"> {{ t("condition") }} Heol </span>
        </label>
    </li> -->
    ...
    <template v-if="withAreas">
        <li v-for="(area, index) in sortedAreas" :key="index">
            ...
            >
                ...
                <template v-if="template.schema.length > 1"
                    >{{
                        template.schema.findIndex(
                            (item) =>
                                item.attachment_uuid === area.attachment_uuid
                        ) + 1
                    }}-</template
                >{{ area.page + 1 }}
            </a>
        ...
        >
            ...
            >
                <IconNewSection :width="20" :stroke-width="1.6" />
                {{ t("draw_new_area") }}
            </a>
        </li>
    </template>
    ...
    computed: {
        ...
        dateFormats() {
            ...
            if (
                ...
                !formats.includes(this.field.preferences.format)
            ) {
    ...
    methods: {
        ...
        copyToAllPages(field) {
            ...
            this.template.documents.forEach((attachment) => {
                const numberOfPages =
                    attachment.metadata?.pdf?.number_of_pages ||
                    attachment.preview_images.length;
