            ...
            >
                <div ref="documents" class="pr-3.5 pl-0.5">
                    <template
                        v-if="
                            !sortedDocuments.length &&
                            (withUploadButton || withAddPageButton)
                        "
                    >
                        <Dropzone
            ...
            >
                            ...
                            <a
                                v-if="
                                    ...
                                    ![
                                        ...
                                        'initials',
                                        'heading',
                                    ].includes(drawField?.type || drawFieldType)
                                "
                                href="#"
                ...
                <div>
                    <Fields
                        ...
                        "
                        @set-draw-type="
                            [(drawFieldType = $event), (showDrawField = true)]
                        "
...
import MobileDrawField from "./mobile_draw_field.vue";
import Document from "./document.vue";
import Logo from "./logo.vue";
import Contenteditable from "./contenteditable.vue";
import DocumentPreview from "./preview.vue";
import DocumentControls from "./controls.vue";
import MobileFields from "./mobile_fields.vue";
import {
    IconPlus,
    IconUsersPlus,
    IconDeviceFloppy,
...
export default {
    ...
    computed: {
        ...
        isAllRequiredFieldsAdded() {
            return !this.defaultRequiredFields?.some((f) => {
                return !this.template.fields?.some(
        ...
        sortedDocuments() {
            return this.template.schema.map((item) => {
                const sortedDocs = this.template.documents.find(
    ...
    created() {
        console.log("Fields", JSON.parse(JSON.stringify(this.template)));
        if (
    ...
    methods: {
        ...
        removePendingFields() {
            this.template.fields = this.template.fields.filter((f) => {
                return this.template.schema.find(
                    (item) =>
                        item.attachment_uuid === f.attachment_uuid &&
                        item.pending_fields
                );
            });
        ...
        handleAreaArrows(event) {
            ...
            if (event.key === "ArrowRight" && event.altKey) {
                area.w = Math.min(
                    Math.max(area.w + diff / rect.width, 0),
                    1 - area.x
                );
                ...
                area.h = Math.min(
                    Math.max(area.h - diff / rect.height, 0),
                    1 - area.y
                );
                ...
                area.x = Math.min(
                    Math.max(area.x + diff / rect.width, 0),
                    1 - area.w
                );
                ...
                area.y = Math.min(
                    Math.max(area.y - diff / rect.height, 0),
                    1 - area.h
                );
        ...
        removeArea(area) {
            ...
            if (!field.areas.length) {
                this.template.fields.splice(
                    this.template.fields.indexOf(field),
                    1
                );
            }
        ...
        onDraw(area) {
            if (this.drawField) {
                if (this.drawOption) {
                    ...
                    if (
                        ...
                        !this.drawField.areas.find(
                            (a) =>
                                a.option_uuid === this.drawField.options[0].uuid
                        )
                    ) {
                        areaWithoutOption.option_uuid =
                            this.drawField.options[0].uuid;
                    }
                ...
                const insertBeforeAreaIndex = this.drawField.areas.findIndex(
                    (a) => {
                        return (
                            a.attachment_uuid === area.attachment_uuid &&
                            a.page > area.page
                        );
                    }
                );
                ...
                let type =
                    pageMask.clientWidth * area.w < 35 ? "checkbox" : "text";
                ...
                if (
                    ...
                    if (previousArea || area.w) {
                        const areaW =
                            previousArea?.w || 30 / pageMask.clientWidth;
                        const areaH =
                            previousArea?.h || 30 / pageMask.clientHeight;
        ...
        onDropfield(area) {
            ...
            if (this.selectedField?.type === field.type) {
                ...
                if (["checkbox"].includes(field.type)) {
                    baseArea = {
                        ...
                        h:
                            (area.maskW / 30 / area.maskW) *
                            (area.maskW / area.maskH),
                    };
                } else if (field.type === "image") {
                    baseArea = {
                        w: area.maskW / 5 / area.maskW,
                        h:
                            (area.maskW / 5 / area.maskW) *
                            (area.maskW / area.maskH),
                    };
                } else if (
            ...
            if (field.type === "heading") {
                this.$nextTick(() => {
                    ...
                    const areaRef = documentRef.pageRefs[
                        area.page
                    ].areaRefs.find(
        ...
        onDocumentRemove(item) {
            if (window.confirm(this.t("are_you_sure"))) {
                this.template.schema.splice(
                    this.template.schema.indexOf(item),
                    1
                );
            }
        ...
        onDocumentReplace(data) {
            ...
            if (data.fields) {
                ...
                this.template.fields.forEach((field) => {
                    [...(field.areas || [])].forEach((area) => {
                        if (
                            ...
                            replaceSchemaItem.attachment_uuid
                        ) {
                            field.areas.splice(field.areas.indexOf(area), 1);
                ...
                this.template.fields = this.template.fields.filter(
                    (f) =>
                        !removedFieldUuids.includes(f.uuid) || f.areas?.length
                );
            }
            ...
            if (!this.template.fields.length) {
                ...
                if (submitterWithoutFields) {
                    ...
                    document.dispatchEvent(
                        new CustomEvent("DOCUMENT_SAVE_RECEIVED", {
                            detail: {
                                type: "ON_DOCUMENT_RECEIVE",
                                data: this.template,
                            },
                        })
                    );
