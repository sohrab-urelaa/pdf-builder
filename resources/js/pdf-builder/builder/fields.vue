<template>
    ...
    >
        <Field
            ...
            :field="field"
            :type-index="
                fields.filter((f) => f.type === field.type).indexOf(field)
            "
            :editable="
                editable &&
                (!fieldsDragFieldRef.value ||
                    fieldsDragFieldRef.value !== field)
            "
    ...
    <div v-if="submitterDefaultFields.length && editable">
        ...
        >
            <div
                v-if="
                    !filteredSubmitterDefaultFields.length &&
                    defaultFieldsSearch
                "
...
import FieldSubmitter from "./field_submitter.vue";
import { IconLock, IconCirclePlus } from "@tabler/icons-vue";
import IconDrag from "./icon_drag.vue";
        ...
        fieldIconsSorted() {
            if (this.fieldTypes.length) {
                ...
                return Object.fromEntries(
                    Object.entries(this.fieldIcons).filter(
                        ([key]) => key !== "heading"
                    )
                );
            }
