    ...
    <div class="modal modal-open items-start !animate-none overflow-y-auto">
        ...
        <div class="modal-box pt-4 pb-6 px-6 mt-20 max-h-none w-full max-w-xl">
            ...
            >
                ...
                <a href="#" class="text-xl" @click.prevent="$emit('close')"
                    >&times;</a
                >
            </div>
            <div>
                ...
                >
                    <a target="_blank" class="link">Available in Pro</a>
                </div>
                        ...
                        >
                            ...
                            <select
                                ...
                                @change="
                                    [
                                        ...
                                        conditionActions(condition).includes(
                                            condition.action
                                        )
                                            ? ''
                                            : (condition.action =
                            ...
                            >
                                ...
                                >
                                    {{
                                        f.name ||
                                        buildDefaultName(f, template.fields)
                                    }}
                                </option>
                            </select>
                            <select
                            ...
                            <select
                                v-if="
                                    conditionField(condition)?.options?.length
                                "
                                class="select select-bordered select-sm w-full bg-white h-11 pl-4 text-base font-normal"
                                required
                            ...
                            >
                                ...
                                <option
                                    v-for="(option, index) in conditionField(
                                        condition
                                    ).options"
                                    :key="option.uuid"
                                    :value="option.uuid"
...
export default {
    ...
    data() {
        return {
            conditions: this.field.conditions?.[0]
                ? JSON.parse(JSON.stringify(this.field.conditions))
                : [{}],
        };
    },
    computed: {
