<template>
    <div class="list-field group mb-2">
        ...
        >
            ...
            >
                ...
                <div v-else-if="editable" class="flex items-center space-x-1">
                    ...
                    <button
                        ...
                        title="Toggle Common Settings"
                        @click="isShowCommonSettings = !isShowCommonSettings"
                    >
                        <IconChevronDown :width="18" :stroke-width="1.6" />
                    </button>
                    <button
            ...
            <template v-if="isShowCommonSettings">
                <CommonFieldSettings :field="field" />
            </template>
        </div>
...
import FormulaModal from "./formula_modal.vue";
import ConditionsModal from "./conditions_modal.vue";
import DescriptionModal from "./description_modal.vue";
import CommonFieldSettings from "./common_field_settings.vue";
import {
    IconRouteAltLeft,
    IconMathFunction,
    IconNewSection,
...
export default {
    ...
    methods: {
        buildDefaultName(field, fields) {
            if (field.type === "payment" && field.preferences?.price) {
                ...
                if (this.field.type === "heading") {
                    ...
                    return `${this.fieldNames[field.type]} ${suffix} ${
                        typeIndex + 1
                    }`;
                }
            }
        ...
        maybeFocusOnOptionArea(option) {
            const area = this.field.areas.find(
                (a) => a.option_uuid === option.uuid
            );
