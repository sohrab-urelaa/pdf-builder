...
export const getPagesScrollTopByIndex = (
    ...
    return pageSizes
        ...
        .reduce(
            (acc, cur) =>
                acc + (cur.height * ZOOM + RULER_HEIGHT * scale) * scale,
            0
        );
};
...
const handlePositionSizeChange = (
    ...
    const calcBounds = (v: any, min: number, max: number) =>
...
export const changeSchemas = (args: {
    ...
    const { objs, schemas, basePdf, pluginsRegistry, pageSize, commitSchemas } =
        args;
    const newSchemas = objs.reduce((acc, { key, value, schemaId }) => {
        ...
        if (key === "type") {
            ...
            ["position.x", "position.y", "width", "height"].includes(key)
        ) {
