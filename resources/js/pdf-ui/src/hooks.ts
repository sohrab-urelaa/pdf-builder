...
export const useUIPreProcessor = ({
    ...
    return {
        ...
        refresh: (template: Template) =>
            init({ template, size }).then(
                ({ pageSizes, scale, backgrounds }) => {
                    setPageSizes(pageSizes),
                        setScale(scale),
                        setBackgrounds(backgrounds);
                }
...
export const useInitEvents = ({
    ...
    const initEvents = useCallback(() => {
        ...
        initShortCuts({
            ...
            paste: () => {
                if (
                    !copiedSchemas.current ||
                    copiedSchemas.current.length === 0
                )
                    return;
                ...
                const pasteSchemas = copiedSchemas.current.map((cs) => {
                    ...
                    const position = {
                        x:
                            p.x + 10 > ps.width - width
                                ? ps.width - width
                                : p.x + 10,
                        y:
                            p.y + 10 > ps.height - height
                                ? ps.height - height
                                : p.y + 10,
                    };
