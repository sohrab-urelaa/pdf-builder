...
const TemplateEditor = ({
    ...
    const [hoveringSchemaId, setHoveringSchemaId] = useState<string | null>(
        null
    );
            ...
            <DndContext
                onDragEnd={(event) => {
                    ...
                    const canvasLeftOffsetFromPageCorner =
                        pageRect.left -
                        dragStartLeft +
                        scaleDragPosAdjustment(20, scale);
                    const canvasTopOffsetFromPageCorner =
                        pageRect.top - dragStartTop;

                    const moveY =
                        (event.delta.y - canvasTopOffsetFromPageCorner) / scale;
                    const moveX =
                        (event.delta.x - canvasLeftOffsetFromPageCorner) /
                        scale;
