...
class SubmitTemplateController extends Controller
{
    public function uploadTemplate(Request $request)
    {
        try {
            ...
            $template = PdfTemplate::with("owner")
                ->select('user_id', 'id', "created_at", 'updated_at', "description", "title", "submission_count")
                ->find($template_id);
            ...
            $current_submission_count = $template["submission_count"] ?? 0;

            $submission_limit_count = PlanHistoryController::getSubmissionLimitCountForCompany($template["owner"]);


            Log::info("LIMIT_SUBMISSION", ["count" => $submission_limit_count, "submit_count" => $current_submission_count]);
            if ($current_submission_count >= $submission_limit_count) {
                return response()->json(["success" => false, "message" => "Submission limit has been reached, ask your template owner to upgrade the plan"]);
            }
            $template->submission_count = $current_submission_count + 1;
    ...
    private function signPDF($certificate_path, $pdf_path, $originalFileName, $certificate, $submitter)
    {
        ...
        Log::info("RECEIVED RESPONSE FROM NODE SERVER");
        if ($response->getStatusCode() === 200) {
            $fileContent = $response->getBody()->getContents();
            $filePath = 'signed_pdfs/' . $originalFileName;
            Storage::disk('public')->put($filePath, $fileContent);

            Log::info("UPLOADED NODE SERVER RETURNED PDF");
            return [
                "success" => true,
                "result" => $filePath
            ];
